...
const UserManagement = () => {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(u => {
      ...
      const matchesTab = activeTab === 'active' ? u.isActive : !u.isActive;
  ...
  const handleDelete = async (user) => {
    const result = await Swal.fire({
      ...
      text: t('users.delete_confirm.text', { name: `${user.firstName} ${user.lastName}` }),
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: t('users.delete_confirm.confirm'),
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEditing || formData.password) {
      if (formData.password !== formData.confirmPassword) {
        return toast.error(t('users.messages.password_mismatch'));
      }
      const isValidPassword = passwordRules.every(rule => rule.regex.test(formData.password));
      if (!isValidPassword) {
        return toast.error(t('users.messages.password_invalid'));
      }
    }
    ...
    const savePromise = isEditing 
      ? userService.updateUser(formData.id, payload) 
